...
```
上面的action方法，调用了静态方法test1和私有方法test2，因为静态方法和私有方法不能被重写，因此这个action方法需要调用的方法是明确的，固定的。在类解析阶段，这个action方法中的具体的方法调用就会明确的在方法区内保存。所有实例对象不需要自己再去做什么操作就可以调用这个action方法。
...
大概的意思就是，当前栈会保存调用者的局部变量表和操作数栈，方法正常执行完毕之后，
会恢复调用者局部变量表和操作数栈，同时把返回值压如操作数栈。
```

虽然jvm规范说一个方法的栈帧里面会存有调用者的局部变量表和操作数栈信息，但是实际的虚拟机中两个栈帧会公用某一部分，避免信息的重复复制。所以从物理存储上来讲，栈帧中的部分内容并不完全是一个方法栈帧独有的，可能会和调用者栈帧有所重合。
### 总结
+ 虚拟机栈由栈帧组成，方法的调用和结束对应栈帧的出入栈行为。
+ 栈帧由局部变量表、操作数栈、动态链接、返回地址组成。
+ 局部变量表存放this引用、入参和局部变量，如果是静态方法则没有this引用。
+ 存在操作数栈是因为jvm虚拟机指令集是基于栈的，这样的指令对底层硬件的依赖更小，有利于跨平台。
+ 存在动态链接是因为有些方法的调用只有在运行期间才能确定，它是java多态的实现原理。
+ 返回地址保存了方法结束的信息，不仅仅是一个地址，还有调用者的操作数栈和局部变量表，用于在方法结束时恢复调用者的状态，向调用者操作数栈中压入方法返回值。
